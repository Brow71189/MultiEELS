...
#            state_controller.initialize_state()
    ...
    def _set_up_multi_acquire(self, settings: dict, parameters: list):
        multi_acquire = MultiEELS.MultiEELS()
    ...
    def test_data_intensity_scale_is_correct_for_summed_frames(self):
        ...
        parameters = [{'index': 0, 'offset_x': 0, 'offset_y': 0, 'exposure_ms': 5, 'frames': 10},
                      ...
                      {'index': 2, 'offset_x': 320, 'offset_y': 0, 'exposure_ms': 16, 'frames': 1}]
        multi_acquire = self._set_up_multi_acquire(settings, parameters)
        multi_acquire._MultiEELS__active_settings = copy.deepcopy(multi_acquire.settings)
        multi_acquire._MultiEELS__active_spectrum_parameters = copy.deepcopy(multi_acquire.spectrum_parameters)
        for parms in parameters:
            ...
            multi_acquire._MultiEELS__queue.put(data_dict)
        multi_acquire._MultiEELS__acquisition_finished_event.set()
        processed_data = []
